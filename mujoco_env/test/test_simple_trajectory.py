...
使用方法:
    # 使用系统Python以支持ROS2功能
    /usr/bin/python3 test_simple_trajectory.py
...
class ContinuousArmController:
    ...
    def __init__(self, env: RLCartesianEnvWithIK, enable_ros_control: bool = False, enable_teaching_recording: bool = False, enable_rosbag: bool = False):
        self.env = env
        ...
        if self.enable_rosbag:
            try:
                ...
                self.rosbag_recorder = RosbagRecorder(
                    output_dir="/home/tower/Documents/Arm_Project/rosbag_data",
                    topics=[
...
def test_integrated_trajectory_control():
    ...
    try:
        ...
        env_xml_path = os.path.join(os.path.dirname(__file__), '../robot_model/exp/env_robot_torque.xml')
        ...
        controller = ContinuousArmController(
            ...
            enable_teaching_recording=False,
            enable_rosbag=True
        )
        ...
        # 6. RGBD图像发布循环
        try:
            print("\nStarting RGBD image publishing loop (15Hz)")
            ...
            while True:
                ...
                target_duration = 1/15  # 16.7ms (60Hz)
